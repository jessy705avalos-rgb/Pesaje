...
fun HistorialTrailerScreen(
    ...
    Column(modifier = modifier.fillMaxSize()) {
        ...
        TablaRegistrosTrailer(registros = registros)
    }
...
private fun TablaRegistrosTrailer(registros: List<RegistroPesajeTrailer>) {
    ...
    val maxCaracteresPlacas = remember(registros) {
        val maxEnLista = registros.maxOfOrNull { it.placas.length } ?: 0
        maxOf(9, maxEnLista)
    }
    val anchoPlacas = 110.dp
    ...
    val anchoCarga = ((maxCaracteresCarga * 12) + 10).dp.coerceAtLeast(110.dp)
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        Box(
            ...
            Column(
                ...
                FilaEncabezadoTrailer(
                    anchoPlacas = anchoPlacas,
                    anchoConductor = anchoConductor,
...
@Composable
private fun FilaEncabezadoTrailer(
    anchoPlacas: Dp,
    anchoConductor: Dp,
    anchoCarga: Dp
) {
...
@Composable
private fun FilaRegistroTrailer(
    registro: RegistroPesajeTrailer,
    anchoPlacas: Dp,
    anchoConductor: Dp,
    anchoCarga: Dp
) {
    Row(modifier = Modifier.padding(start = 16.dp, top = 8.dp, bottom = 8.dp)) {
        Text("${registro.id}", modifier = Modifier.width(50.dp).padding(end = 16.dp), maxLines = 1, softWrap = false)
        Text(registro.placas, modifier = Modifier.width(anchoPlacas).padding(end = 5.dp), maxLines = 1, softWrap = false)
